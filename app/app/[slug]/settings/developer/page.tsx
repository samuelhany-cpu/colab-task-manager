...
export default function DeveloperSettingsPage({
  ...
  const generateNewKey = () => {
    ...
    setTimeout(() => {
      setApiKey(
        `ctm_live_${Math.random().toString(36).substring(2, 12)}...${Math.random().toString(36).substring(2, 6)}`,
      );
      setGenerating(false);
    }, 1000);
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="space-y-1">
            ...
            <p className="text-mutedForeground text-lg font-medium">
              Manage API keys and integration settings for{" "}
          ...
          <Card className="md:col-span-2 p-8 md:p-10 border-border/40 shadow-sm bg-card rounded-[2rem] space-y-8">
            <div className="space-y-2">
              ...
              <p className="text-mutedForeground font-medium">
                Use this key to authenticate requests to the Colab Task Manager
                API.
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-3">
                  <div className="relative flex-1 group">
                    <Terminal
                      className="absolute left-4 top-1/2 -translate-y-1/2 text-mutedForeground/40 group-focus-within:text-primary transition-colors"
                      size={18}
                    />
                    <Input
                      value={apiKey}
              ...
              <div className="flex items-center gap-2 px-4 py-3 bg-amber-500/5 border border-amber-500/20 rounded-xl">
                ...
                <p className="text-xs font-bold text-amber-700 leading-tight">
                  Keep your API key secret. Do not share it or commit it to
                  version control.
                </p>
              </div>
            </div>
  ...
  -H "Authorization: Bearer \${YOUR_API_KEY}"`}
          ...
          <div className="space-y-6">
            <Card className="p-8 border-border/40 shadow-sm bg-card rounded-2xl space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-mutedForeground leading-relaxed">
                  Configure webhooks or download our official SDKs for Node.js,
                  Python, and Go.
                </p>
              </div>
              <Button
                variant="ghost"
                className="w-full justify-start gap-2 font-bold px-0 hover:bg-transparent hover:text-primary"
              >
                Configure Webhooks
            ...
            <Card className="p-8 border-primary/20 bg-primary/5 rounded-2xl space-y-4 border-dashed">
              <Badge className="bg-primary text-primary-foreground font-black text-[9px] uppercase tracking-tighter">
                Coming Soon
              </Badge>
