    ...
    if (loading)
        return (
            <div className="h-screen flex flex-col items-center justify-center gap-4 bg-muted/30">
                ...
                <p className="font-medium text-mutedForeground">Loading team members...</p>
            </div>
        );
            ...
            <header className="max-w-6xl mx-auto space-y-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                    <div className="space-y-1">
                        <h1 className="text-4xl font-extrabold tracking-tight text-foreground">Team Members</h1>
                            ...
                            {members.map((member) => (
                                <div key={member.id} className="p-6 flex items-center justify-between hover:bg-muted/10 transition-colors">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="font-bold text-foreground">{member.user.name || "Invite Pending"}</div>
                                            <div className="text-xs text-mutedForeground font-medium">{member.user.email}</div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-6">
                                        <Badge variant="secondary" className="px-3 py-1 text-[10px] font-black uppercase tracking-widest bg-primary/5 text-primary border-none">
                                            {member.role}
                                        </Badge>
                                        <Button variant="ghost" size="icon" className="text-mutedForeground">
                ...
                <div className="lg:col-span-4 space-y-6">
                    <Card className="p-8 border-border/40 shadow-sm bg-card rounded-2xl">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-mutedForeground font-medium">Add new collaborators</p>
                            </div>
                        </div>

                        <form onSubmit={handleInvite} className="space-y-4">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-mutedForeground ml-1">Email Address</label>
                                <div className="relative">
                                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-mutedForeground/50" size={16} />
                                    <Input
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-mutedForeground ml-1">Member Role</label>
                                <div className="flex gap-2">
                                    <Button type="button" variant="secondary" className="flex-1 h-12 rounded-xl text-xs font-bold gap-2">
                                        <Users size={14} />
                                        Member
                                    </Button>
                                    <Button type="button" variant="ghost" className="flex-1 h-12 rounded-xl text-xs font-bold gap-2 border border-border/40">
                                        <Shield size={14} />
                                        Admin
                                    </Button>
                                </div>
                    ...
                    <Card className="p-8 border-border/40 bg-gradient-to-br from-primary/5 to-transparent rounded-2xl">
                        ...
                        <p className="text-xs text-mutedForeground leading-relaxed mb-6">
                            You are currently on the <span className="text-foreground font-bold italic">Free Plan</span>.
                            You can add up to <span className="text-foreground font-bold">5 team members</span>.
                        </p>
                        <Button variant="secondary" className="w-full rounded-xl font-bold h-10 text-xs">
                            View Plan Details
                        </Button>
                    </Card>
                </div>
