  ...
  try {
    ...
    const results = [
      ...
      ...files.map((f) => ({
        ...
        url: `/app/${workspaceSlug || "default"}/files`,
      })),
      ...messages.map((m) => ({
        id: m.id,
        type: "MESSAGE",
        ...
        subtitle: `from ${m.sender.name || "User"} in ${m.project?.name || "DM"}`,
        url: `/app/${m.project?.workspace.slug || workspaceSlug}/chat?channel=${m.projectId || "personal"}`,
      })),
    ];
