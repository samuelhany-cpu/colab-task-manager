...
import { Search, Command, X, FileText, LayoutGrid, CheckCircle2, Loader2, ArrowRight } from "lucide-react";
...
export default function GlobalSearchModal({
    ...
    const fetchResults = useCallback(async (q: string) => {
        ...
        try {
            setLoading(true);
            const res = await fetch(`/api/search?q=${encodeURIComponent(q)}&workspaceSlug=${workspaceSlug}`);
            if (res.ok) {
                ...
                <div className="max-h-[450px] overflow-y-auto custom-scrollbar">
                    {loading ? (
                        <div className="p-12 flex flex-col items-center justify-center gap-4 text-mutedForeground">
                            ...
                            <p className="text-sm font-bold uppercase tracking-widest">Searching the workspace...</p>
                        </div>
                    ) : results.length > 0 ? (
                        <div className="p-2">
                            {results.map((result, index) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        index === selectedIndex ? "bg-primary text-primary-foreground shadow-lg scale-[1.01]" : "hover:bg-muted"
                                    )}
                                >
                                    <div className={cn(
                                        ...
                                        {result.type === "TASK" ? <CheckCircle2 size={20} /> :
                                            result.type === "PROJECT" ? <LayoutGrid size={20} /> :
                                                <FileText size={20} />}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className={cn(
                                            "text-xs truncate",
                                            index === selectedIndex ? "text-white/70" : "text-mutedForeground"
                                        )}>
                                            {result.subtitle}
                                        </p>
                                    </div>
                        ...
                        <div className="p-12 text-center text-mutedForeground">
                            <p className="text-lg font-black uppercase tracking-widest mb-2">No matches found</p>
                            <p className="text-sm font-medium opacity-60">We couldn't find anything matching "{query}"</p>
                        </div>
                    ) : (
                        <div className="p-12 text-center text-mutedForeground/40">
                            <div className="flex justify-center gap-8 mb-4">
                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-12 h-12 rounded-2xl bg-muted/50 flex items-center justify-center"><CheckCircle2 size={24} /></div>
                                    <span className="text-[10px] font-black uppercase tracking-tighter">Tasks</span>
                                </div>
                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-12 h-12 rounded-2xl bg-muted/50 flex items-center justify-center"><LayoutGrid size={24} /></div>
                                    <span className="text-[10px] font-black uppercase tracking-tighter">Projects</span>
                                </div>
                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-12 h-12 rounded-2xl bg-muted/50 flex items-center justify-center"><FileText size={24} /></div>
                ...
                <div className="p-3 border-t border-border bg-muted/10 flex items-center justify-between px-6">
                    <div className="flex gap-4">
                        <div className="flex items-center gap-1 text-[10px] font-bold text-mutedForeground">
                            <span className="px-1.5 py-0.5 rounded bg-muted border border-border">↑↓</span> Navigate
                        </div>
                        <div className="flex items-center gap-1 text-[10px] font-bold text-mutedForeground">
                            <span className="px-1.5 py-0.5 rounded bg-muted border border-border">Enter</span> Select
                        </div>
                    </div>
                    <button
                        onClick={onClose}
